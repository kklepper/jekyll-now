...
4. **Examples 4 and 5**: For the use of variables, this same constraint of uniqueness applies to the part up to `VARTOKEN` used to enclose these (which is a good idea anyway to see if a variable is empty, see example) -- reason: `grep` looks for the original line and will not recognize the substitution (which we do not know), so the code has to stop here as well. Instead, we show the variables in the next line with the prefix `>>>>>> variable substitution`. 
